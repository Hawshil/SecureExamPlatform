...
namespace SecureExamPlatform.Tests.Core
{
    public class ExamSessionManagerTests : IDisposable
    {
        ...
        public ExamSessionManagerTests()
        {
            ...
            _baseDir = Path.Combine(Path.GetTempPath(), $"SecureExamTests_{Guid.NewGuid()}");
            _examsPath = Path.Combine(_baseDir, "Exams");

            // Create directories
            Directory.CreateDirectory(_baseDir);
            Directory.CreateDirectory(_examsPath);
            Directory.CreateDirectory(Path.Combine(_baseDir, "LocalAppData"));

            // Set environment variable to use our test directory
            Environment.SetEnvironmentVariable("LOCALAPPDATA", Path.Combine(_baseDir, "LocalAppData"));

            // Create test exam file before creating session manager
        ...
        private void CreateTestExam()
        {
            ...
            Directory.CreateDirectory(Path.GetDirectoryName(_examFilePath));
            ...
            if (!File.Exists(_examFilePath))
            {
                throw new Exception($"Failed to create test exam file at {_examFilePath}");
            }
        }
        ...
        public async Task StartSession_WithValidCredentials_ShouldCreateSession()
        {
            ...
            // Verify exam file exists
            Assert.True(File.Exists(_examFilePath), $"Test exam file does not exist at {_examFilePath}");
        ...
        public async Task EndSession_ShouldTerminateActiveSession()
        {
            ...
            Assert.True(startResult.Success, $"Failed to start initial session: {startResult.ErrorMessage}");
        ...
        public async Task StartSession_WithExistingSession_ShouldFail()
        {
            ...
            Assert.True(firstResult.Success, "First session failed to start");

            // Try to start another session with a different student
        ...
        public async Task GetSessionEvents_ShouldTrackSessionActivity()
        {
            ...
            var startResult = await _sessionManager.StartSession(studentId, accessToken, examId);
            Assert.True(startResult.Success, $"Failed to start session: {startResult.ErrorMessage}");
